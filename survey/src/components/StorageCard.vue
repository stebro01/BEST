...
<div>
  <q-item class="my-btn-item" :class="{'bg-grey-4': checked}">
    ...
    <q-item-section no-wrap>
      ...
      <q-item-label caption style="font-size: 10px; max-height: 1em" class="overflow-hidden" >
      {{item.cda.date }}
      </q-item-label>
    ...
    <q-item-section side>
       <div class="text-grey-8 q-gutter-xs">
         <q-btn flat dense round icon="preview" @click="view_item(index)" :data-cy="'btn_preview_'+index">
        ...
        <q-btn size="12px" flat dense round icon="more_vert" data-cy="btn_options" >
          <q-menu cover auto-close>
            <q-list>
              ...
              <!-- EXPORT -->
              <q-item  class="my-btn text-center" data-cy="back_root" clickable @click="export_item(index)">
                <q-item-section avatar> <q-icon :name="TEXT.btn.export.icon" /> </q-item-section>
                <q-item-section >{{TEXT.btn.export.label}}</q-item-section>
              </q-item>
              <!-- EXPORT -->
              <q-item  class="my-btn text-center" data-cy="back_root" clickable @click="export_item_encrypted(index)">
                <q-item-section avatar> <q-icon :name="TEXT.btn.export.icon2" /> </q-item-section>
                <q-item-section >{{TEXT.btn.export.label}} ({{TEXT.btn.export.encrypt}})</q-item-section>
              </q-item>
              <!-- EXPORT CLOUD/NOTION -->
              <q-item  v-if="$store.getters.SETTINGS.use_notion" class="my-btn text-center" data-cy="back_root" clickable @click="export_cloud(index)">
                <q-item-section avatar> <q-icon name="cloud" /> </q-item-section>
                <q-item-section >{{TEXT.btn.export_cloud}}</q-item-section>
              </q-item>
              <q-separator></q-separator>
              <!-- DELETE -->
              <q-item  class="my-btn text-center" data-cy="back_root" clickable @click="remove(index)">
                <q-item-section avatar> <q-icon :name="TEXT.btn.delete.icon" /> </q-item-section>
