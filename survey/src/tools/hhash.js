...
export function verify(jsondata, hash) {
    ...
    var isVerified = verify.verify(JSON.stringify(jsondata), hash.signature, CryptoJS.SHA256);
    return isVerified
}
...
function create_aes_key() {
    ...
    var key128Bits = CryptoJS.PBKDF2(passphrase, salt, { keySize: 128/32 }).toString();
    return key128Bits
}
...
export function decrypt(payload, privateKey) {
    ...
    if (decrypted_key === null) return 'could not decrypt key'
