  ...
  <q-page class="page-size">
    <div class="column items-center" style="height: 100%">
        ...
        <q-scroll-area class="shadow-1 my-form">
          <div class="row q-pa-md justify-around q-gutter-md" data-cy="items">
            <div class="col-auto" v-for="(item, index) in QUEST_LIST" :key="'item_'+index" :data-cy="'item_'+index">
              ...
              <StorageCard v-if="FILTER_ON === false || FILTER.text === null || item.info.PID.includes(FILTER.text) > 0 || item.info.title.includes(FILTER.text) > 0"
                ...
                @export_item="export_item(item.info.uid)" @export_item_encrypted="export_item_encrypted(item.info.uid)" @remove="remove(item.info.uid)" @view_item="view_item(item.info.uid)" />
            </div>
          </div>
      ...
      <div v-if="QUEST_LIST.length > 0" class="col-3 col-md-2 text-center q-gutter-md  justify-around" style="width: 100%">
          <!-- SELECTALL -->
          <MYBUTTON v-if="!somethingselected" :icon="TEXT.btn.select_all.icon" :label="TEXT.btn.select_all.label" @click="selectall(true)" />
          <MYBUTTON v-if="!somethingselected" :icon="TEXT.btn.import2.icon" :label="TEXT.btn.import2.label" @click="$router.push({name: 'importQuest'})"/>
          <!-- DESELECT -->
          <MYBUTTON v-if="somethingselected" :icon="TEXT.btn.deselect.icon"  @click="selectall(false)" :label="TEXT.btn.deselect.label"/>
          <!-- EXPORT SELECTION -->
          <MYBUTTON v-if="QUEST_LIST.length > 0 && somethingselected" :icon="TEXT.btn.selection_export.icon" @click="exportselection" data-cy="btn_export_all" :label="TEXT.btn.selection_export.label" />
          <!-- DELETE SELCTON -->
          <MYBUTTON v-if="QUEST_LIST.length > 0 && somethingselected" :icon="TEXT.btn.selection_delete.icon" @click="deleteselection" :label="TEXT.btn.selection_delete.label" />
          <!-- DEBUG PRINT  -->
      ...
      <div v-else class="col-2 text-center " data-cy="no_entry">
        ...
        <div>
          <MYBUTTON  :icon="TEXT.btn.import2.icon" :label="TEXT.btn.import2.label" @click="$router.push({name: 'importQuest'})"/>
        </div>
      </div>
...
<script>
  ...
  export default {
    ...
    mixins: [myMixins],
    components: {
      TABLEVIEW: () => import('src/components/TableView'),
      StorageCard: () => import('src/components/StorageCard'),
      BACKBUTTON: () => import('src/components/BackButton'),
      MYBUTTON: () => import('src/components/MyButton'),
      FILTERSTORAGE: () => import('src/components/FilterStorage.vue')
    },
    data() {
      return {
        medium: false,
    ...
    methods: {
      ...
      export_item(uid) {
        ...
        this.$store.dispatch('storage_export', [uid]).then(val => {}).catch(err => {
          log({
            error: 'export_item',
            data: err
          })
      ...
      export_item_encrypted(uid) {
        ...
        this.$store.dispatch('storage_encrypted_export', payload).then(val => {}).catch(err => {
          log({
