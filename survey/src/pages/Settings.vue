        ...
        <q-scroll-area class="my-form">
          <div class="row q-pa-md justify-around q-gutter-sm">
            ...
            <q-expansion-item data-cy="btn_description" expand-separator :icon="TEXT.settings.export.icon"
              :label="TEXT.settings.export.label" :caption="`Format: ${$store.getters.SETTINGS.export_format}`"
              class="my-settings-item">
              <div class="row text-center">
                <div class="col-12">
                  <q-btn-toggle flat v-model="$store.getters.SETTINGS.export_format" toggle-color="primary" :options="TEXT.settings.export.options" />
                </div>
                <div class="col-12 text-caption">
                  <span v-if="$store.getters.SETTINGS.export_format === 'html'">{{TEXT.settings.export.description.html}}</span>
                  <span v-else-if="$store.getters.SETTINGS.export_format === 'json'">{{TEXT.settings.export.description.json}}</span>
                  <span v-else-if="$store.getters.SETTINGS.export_format === 'CDA'">{{TEXT.settings.export.description.cda}}</span>
                </div>
            ...
            <q-expansion-item data-cy="btn_description" expand-separator :icon="TEXT.settings.user.icon"
              ...
              <div v-if="user_details" class="row q-ma-xs shadow-1 q-pa-sm">
                <div class="text-caption my-annotation-text" v-html="TEXT.settings.user.uid_description">
                </div>
              ...
              <div v-if="user_details" class="row justify-right text-grey-8 q-mt-md">
                <q-btn no-caps icon="file_upload" flat size="xs" class="col " @click="view_import = true">{{TEXT.btn.user.label_import}}</q-btn>
                <q-btn no-caps icon="refresh" flat size="xs" class="col " @click="user_new()">{{TEXT.btn.user.label_new}}</q-btn>
                <q-btn no-caps icon="file_download" flat size="xs" class="col " @click="view_export = true">{{TEXT.btn.user.label_export}}</q-btn>
              </div>
            ...
            <q-item clickable v-ripple class="my-settings-item" @click="$router.push({name: 'questman'})">
              ...
              <q-item-section>
                ...
                <q-item-label caption>{{TEXT.settings.questman.description}}</q-item-label>
              </q-item-section>
            </q-item>
            ...
            <q-item clickable v-ripple class="my-settings-item" @click="$router.push({name: 'encrypt'})">
              ...
              <q-item-section>
                <q-item-label>{{TEXT.settings.encryption.label}}</q-item-label>
                <q-item-label caption>{{TEXT.settings.encryption.description}}</q-item-label>
              </q-item-section>
            </q-item>
...
<script>
  ...
  export default {
    components: {
      ...
      USEREXPORT: () => import('src/components/User_export'),
      USERIMPORT: () => import('src/components/User_import')
    },
    mixins: [myMixins],
    data() {
      return {
