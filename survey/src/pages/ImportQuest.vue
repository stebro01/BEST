  ...
  <q-page data-cy="page_settigns" class="page-size">
    <div class="column items-center" style="height: 100%">
      ...
      <div class="col text-center" v-if="file !== null">
        <MYBUTTON  @click="loadFile()" :label="TEXT.btn.import" />
  ...
  export default {
    ...
    methods: {
      ...
      readhtml(e) {
        ...
        if (document === undefined) return this.show_error('keine gültigen Daten gefunden')        
      ...
      readjson(e) {
        ...
        // json als encrypted dokument
          ...
          let decrypted_text = decrypt({encrypted_data: json.encrypted_data, encrypted_key: json.encrypted_key}, this.keyPair.privateKey)
          if (decrypted_text === null || decrypted_text === undefined || decrypted_text === 'could not decrypt key') return this.show_error(decrypted_text)
          // else
        ...
        // eventuell ein ARRAY mit encrypted data? neu: 202204
          ...
          json.forEach(j => {
            if (j.encrypted_data !== undefined && j.encrypted_key !== undefined) {
              status = true
              let decrypted_text = decrypt({encrypted_data: j.encrypted_data, encrypted_key: j.encrypted_key}, this.keyPair.privateKey)
        ...
        //ELSE ERROR
        this.show_error('invalid file')
        
      },

      importDocument(document) {
          this.$store.dispatch('storage_add_from_file', document)
          .then(res => {
            this.$q.notify({
