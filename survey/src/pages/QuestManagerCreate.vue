...
export default {
  ...
  components: {
    ...
    CREATEITEM: () => import('src/components/CreateItem'),
    CREATERESULTS: () => import('src/components/CreateResults'),
    PREVIEWITEM: () => import('src/components/PreviewItem'),
  ...
  methods: {
    ...
    updateID() { //loop through the quest and update the ID for each questions
      ...
      this.content.items.forEach(item => {
        switch(item.type) {
          case 'separator':
            this.$set(item, 'id', undefined)
            this.$set(item, 'coding', undefined)
            break
          case 'multiple_radio':
            let local_id = []
            if (item.options !== undefined && item.options.questions !== undefined) {
              ...
              this.$set(item, 'id', JSON.stringify(local_id))
            }
            break
          default:
            this.$set(item, 'id', id)
    ...
    updateItem(payload, item) {
      this.$set(item, payload.field, payload.value)
    ...
    updateResult(payload) {
      this.$set(this.content, payload.field, payload.value)
      this.date_str = Date.now()
    ...
    moveItem(action, index) {
      ...
      switch (action) {
        default:
          ...
          this.$set(this.content.items, index, this.content.items[index-1])
          this.$set(this.content.items, index-1, tmp)
          break
      }
