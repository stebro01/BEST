    ...
    <div class="column items-center" style="height: 100%">
      ...
      <div class="col">
        ...
        <q-scroll-area class="q-ma-xs shadow-1 my-form">
          <q-list v-if="FILTERED_LIST !== undefined && FILTERED_LIST !== null" bordered separator class="quest_list" data-cy="questlistRoot">
            <div  v-for="(item, index) in FILTERED_LIST" :key="item + index + date_str">
      ...
      <div class="col-2 items-center row justify-around" style="width: 100%">
        <!-- PROCEED ZUM Fragebogen -->
          <span v-if="count_selected===0" class="text-grey text-center" v-html="TEXT.select_quest.empty"></span>
          <!-- DESELCT -->
          <MYBUTTON v-if="count_selected>0"  :label="TEXT.btn.deselect.label" @click="deselectAll" data-cy="deselect" />
            <!-- GOTOQUEST -->
          <MYBUTTON v-if="count_selected===1" :label="TEXT.btn.toquest" @click="gotoquest" data-cy="btn_gotoquest" />
          <!-- GOTOPRESET -->
          <MYBUTTON v-if="count_selected>1" data-cy="btn_gotopreset" :label="TEXT.btn.make_preset" @click="gotopreset()" />
      </div>
...
export default {
  name: 'SelectQuestionnaire',
    ...
    FilterQuest: () => import('src/components/Filter'),
  ...
  methods: {
    ...
    set_index(index){
      this.$set(this.INDEX, index, !this.INDEX[index])
    },
    deselectAll() {
      for (let i = 0; i < this.INDEX.length; i++) {
        this.$set(this.INDEX,i,false)
        }
    },
    gotopreset() {
      const presets = []
      for (let i = 0; i<this.INDEX.length; i++) if (this.INDEX[i] === true) presets.push(this.FILTERED_LIST[i])
      this.$router.push(`preset/${JSON.stringify({presets: presets, mode: "new_preset"})}`)
    },
    gotoquest(){
      var quest_label = ''
      for (let i = 0; i<this.INDEX.length; i++) if (this.INDEX[i] === true) {
        quest_label = this.FILTERED_LIST[i]
      }
      this.$router.push(`quest/${encodeURIComponent(JSON.stringify({presets: quest_label, mode: "single"}))}`)
    }
  }
}
</script>
