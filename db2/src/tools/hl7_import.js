...
export async function importHL7toObject(cda, VIEW_CONCEPT) {
  ...
  if (cda.subject.display) PATIENT = {
...
export async function addHL7ObjectToDB(DATA, VIEW_PATIENT, VIEW_VISIT, VIEW_OBSERVATION) {
  if (!DATA || !VIEW_PATIENT || !VIEW_VISIT || !VIEW_OBSERVATION) return {
    status: false,
  ...
  if (DATA.VISITS.length !== DATA.OBSERVATIONS.length) return {
    status: false,
    error: error_codes.invalid_payload
  }
  ...
  for (let visit of DATA.VISITS) {
    ...
    if (!res_visit) res_visit = await VIEW_VISIT.create({
      ...visit,
...
async function _get_FIX_VISIT(DATA, VIEW_VISIT) {
  if (!DATA.FORCE_VISIT) return undefined
    ...
    if (res_visit.data.length > 0) return {
      status: true,
