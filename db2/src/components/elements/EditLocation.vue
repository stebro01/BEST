        ...
        <q-card-section>
          <q-form>
            <q-markup-table>
              <tbody>
                <!-- CODE -->
                <tr><td>CODE_CD🔑</td><td colspan="3"><q-input :disable="mode === 'edit'" dense v-model="localData.CODE_CD" @blur="dataChanged()"/> </td></tr>
                 <!-- NAME_CHAR -->
                 <tr><td>Name</td><td colspan="3"><q-input dense v-model="localData.NAME_CHAR" @blur="dataChanged()"/> </td></tr>
                 <!-- Details -->
                 <tr><td>Details</td><td colspan="3"><q-input dense v-model="localData.LOOKUP_BLOB" @blur="dataChanged()"/> </td></tr>
              </tbody>
            </q-markup-table>
          </q-form>
        </q-card-section>
...
export default {
  ...
  methods: {
    ...
    dataChanged() {
      ...
      if (!this.localData || !this.localData.CODE_CD || !this.localData.NAME_CHAR) return
