...
export default {
  ...
  methods: {
    loadUser(USER_ID) { 
      this.$store.dispatch('searchDB', {query_string: {USER_ID: USER_ID}, table: 'USER_MANAGEMENT'})
      .then(res => {
        this.formData = {USER_ID: res[0].USER_ID, USER_CD: res[0].USER_CD, NAME_CHAR: res[0].NAME_CHAR, USER_BLOB: res[0].USER_BLOB, COLUMN_CD: res[0].COLUMN_CD}
