      ...
      <template v-slot:main>
        <div class="col-12" v-if="!patient_data">
          <q-tabs style="z-index: 10" v-model="selected_import_method">
            <q-tab
              v-for="(item, ind) in options_import"
              :key="ind + 'opt'"
              :name="item.value"
              :label="item.label"
              no-caps
        ...
        <div v-if="!patient_data" class="text-center">
          <SELECT_FILE
            ...
            :label="`${TEXT.select_file} (${ACCEPT_FILETYPE})`"
        ...
        <div v-if="patient_data" class="column items-center">
          <q-card class="my-card text-center">
            <q-card-section>
              <q-banner dense inline-actions class="bg-green-3"
                >Daten gefunden
              ...
              <q-banner
                v-if="info_patientcd_doesnot_match"
                dense
                inline-actions
                class="bg-red-1"
              >
                ...
                <template v-slot:action>
                  <q-btn
                    ...
                    color="black"
                    icon="close"
                    @click="info_patientcd_doesnot_match = false"
                  />
                </template>
              </q-banner>
            </q-card-section>
            ...
            <q-card-actions align="center">
              <q-btn
                ...
                rounded
                no-caps
                @click="saveToDB(patient_data)"
              >
                in DB speichern
                <q-tooltip v-if="mode_visits === 'new'"
                  >Die Daten werden als neue Visite(n) hinzugefügt</q-tooltip
                >
                <q-tooltip v-else
                  >Die Daten werden der aktuellen Visite hinzugefügt</q-tooltip
                >
              </q-btn>
            </q-card-actions>
          </q-card>
    ...
    <q-dialog v-model="show_csv_found_list" v-if="CSV_TMP_DATA">
      <q-card class="my-card">
        <q-card-section class="text-h6"
          >Multiple Patienten gefunden</q-card-section
        >
        <q-card-section class="q-pa-none">
          <div class="text-caption text-center">
            zu importierenden Eintrag auswählen
          </div>
          <q-scroll-area style="height: 300px; width: 100%">
            <q-list bordered>
              <q-item
                v-for="(item, ind) in Object.keys(CSV_TMP_DATA)"
                :key="ind + 'csvfound'"
                clickable
                v-ripple
                @click="_csv_data(CSV_TMP_DATA[item])"
              >
                <q-item-section avatar>{{ ind + 1 }}</q-item-section>
                <q-item-section
                  >{{ CSV_TMP_DATA[item].PATIENT_CD }}:
                  {{ CSV_TMP_DATA[item].VISITS.length }} Visiten</q-item-section
                >
              </q-item>
            </q-list>
          </q-scroll-area>
...
export default {
  ...
  methods: {
    ...
    async importSURVEY(filePath) {
      ...
      // console.log(DATA)
      if (
        DATA.PATIENT.PATIENT_CD !==
        this.$store.getters.PATIENT_PINNED.PATIENT_CD
      )
        this.info_patientcd_doesnot_match = true;
    },
    ...
    removeItem(item) {
      if (item.observation !== undefined && item.visit !== undefined) {
        ...
        if (this.patient_data.OBSERVATIONS[item.visit].length === 0) {
          ...
          if (this.patient_data.VISITS.length === 0)
            this.patient_data = undefined;
        }
