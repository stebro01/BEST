<template>
  <q-page
    class="column text-center q-pa-sm non-selectable"
    style="height: 100%"
  >
    ...
    <div class="col">
      ...
      <div class="absolute-center q-gutter-y-xl">
        <q-btn
          color="primary"
          rounded
          class="my-list-btn"
          label="Datenbank verbinden"
          @click="this.$router.push({ name: 'selectDB' })"
        />

        <q-btn
          class="my-list-btn text-black"
          rounded
          label="neue DB erstellen"
          @click="this.$router.push({ name: 'DBFunctions_CreateDB' })"
        />
      </div>
    </div>
  </q-page>
...
export default {
  ...
  computed: {
    db_is_connected() {
      if (!this.$store.getters.CONNECTED) return false;
      else return true;
    },
