...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      // resolve data
      if (this.obs.CONCEPT_CD)
        this.$store
          .dispatch("getConceptBy_CONCEPT_CD", this.obs.CONCEPT_CD)
          .then((res) => (this.obs.CONCEPT_CD = res));
      if (this.obs.TVAL_CHAR)
        this.$store
          .dispatch("getConceptBy_CONCEPT_CD", this.obs.TVAL_CHAR)
          .then((res) => (this.obs.TVAL_CHAR = res));
      if (this.obs.LOCATION_CD)
        this.$store
          .dispatch("getLookupBy_CODE_CD", this.obs.LOCATION_CD)
          .then((res) => (this.obs.LOCATION_CD = res));
      if (this.obs.PROVIDER_ID)
        this.$store
          .dispatch("getProviderBy_PROVIDER_ID", this.obs.PROVIDER_ID)
          .then((res) => (this.obs.PROVIDER_ID = res));

      // PREPARE OPTIONS
      this.options_location = [];
      this.$store
        .dispatch("searchDB", {
          query_string: { COLUMN_CD: "LOCATION_CD" },
          table: "CODE_LOOKUP",
        })
        .then((res) =>
          res.forEach((r) => {
      ...
      this.$store
        .dispatch("searchDB", {
          query_string: { PROVIDER_PATH: "U", _like: true },
          table: "PROVIDER_DIMENSION",
        })
        .then((res) =>
          res.forEach((r) => {
            this.options_provider.push({
              value: r.PROVIDER_ID,
              label: r.NAME_CHAR,
            });
          })
    ...
    loadConceptSelection() {
      if (this.obs.VALTYPE_CD === "S" && this.obs.CONCEPT_CD) {
        ...
        this.$store
          .dispatch("searchDB", {
            query_string: { CONCEPT_CD: CONCEPT_CD },
            table: "CONCEPT_DIMENSION",
          })
          .then((res) => {
            if (res.length > 0)
              this.$store
                .dispatch("getConceptList", `${res[0].CONCEPT_PATH}\\LA`)
                .then((res) => (this.options_concept_selection = res));
          });
      } else if (this.obs.VALTYPE_CD === "F" && this.obs.CONCEPT_CD)
        this.$store
          .dispatch("getConceptList", "\\SNOMED-CT\\362981000\\260245000\\")
          .then((res) => (this.options_concept_selection = res));
    },
