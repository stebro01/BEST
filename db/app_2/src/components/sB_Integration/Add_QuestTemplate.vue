<template>
  <q-dialog v-model="ACTIVE">
    <q-card class="q-pa-xs z-max">
      ...
      <q-card-section>
        <SELECT_FILE :label="TEXT.add_file_dialog" :accept="'.json'" @save="loadFile($event)"/>
      </q-card-section>
  ...
  methods: {
    ...
    readjson(e) {
      ...
      try {
        ...
        if (!JSON_DATA.short_title || !JSON_DATA.title || !JSON_DATA.description) throw new Error('Fehlerhafter Fragebogen')
    ...
    saveQuestToDB(JSON_DATA) {
      ...
      this.$store.dispatch('addDB', { table: 'CODE_LOOKUP', query_string: new_quest })
      .then(res => {
