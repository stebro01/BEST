<template>
     <q-card  class="my-card q-mt-xl">
          ...
          <q-card-actions class="q-pa-md row justify-center">
            <q-btn v-if="file_to_read" no-caps  align="around" rounded icon="upload_file" class="my-btn" @click="$emit('save', file_to_read)">{{ $store.getters.TEXT.btn.load }} <q-tooltip>{{ $store.getters.TEXT.btn.tooltip.load }}</q-tooltip></q-btn>
          </q-card-actions>
