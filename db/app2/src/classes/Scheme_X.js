...
export class Scheme_X {
     ...
     */
     create(payload) {
        info({method: this._CLASS_NAME, message: 'create', data: JSON.stringify(payload)})
        if (!payload) return {query: undefined, error: error_codes.no_payload}

        // UPDATE_DATE hinzufuegen
        if (this._FIELDS['IMPORT_DATE'] && !payload['IMPORT_DATE']) payload['IMPORT_DATE'] = now()
        ...
        Object.keys(payload).forEach(key => {
            if (this._FIELDS[key]) {
                ...
                else if (typeof(payload[key]) === 'string') VALUES += `"${payload[key]}"`
                else if (typeof(payload[key]) === 'object') VALUES += `"${stringify(payload[key])}"`
        ...
        if (!VALUES || VALUES === '' || !FIELDS || FIELDS === '') return {query: undefined, error: error_codes.invalid_payload}
     ...
     */
    read(payload) {
        info({method: this._CLASS_NAME, message: 'read', data: JSON.stringify(payload)})
        if (!payload) return {query: undefined, error: error_codes.no_payload}
        var EL = ''
     ...
     */
    update(payload) {
        info({method: this._CLASS_NAME, message: 'update', data: JSON.stringify(payload)})
        if (!payload) return {query: undefined, error: error_codes.no_payload}
        if (!payload.set || !payload.where) {
            info({method: this._CLASS_NAME, message: 'update', data: 'payload must be an object: {where: {}, set: {}}'})
            return {query: undefined, error: error_codes.invalid_payload}
        }

        // SETZE EIN UPDATE_DATUM
        if (this._FIELDS['UPDATE_DATE'] && !payload.set['UPDATE_DATE']) payload.set['UPDATE_DATE'] = now()
        ...
        Object.keys(payload.where).forEach(key => {
            if (this._FIELDS[key]) {
                if (WHERE !== '') WHERE += ' AND '
                if (this._FIELDS[key] === 'numeric') WHERE += `"${key}" = ${payload.where[key]}`
        ...
        if (WHERE === '' || SET === '') return {query: undefined, error: error_codes.invalid_payload}
        
        return {query: `UPDATE "${this._TABLE_NAME}" SET ${SET} WHERE ${WHERE}`, error: undefined}
    }
     ...
     */
    delete(payload) {
        info({method: this._CLASS_NAME, message: 'delete', data: JSON.stringify(payload)})
        if (!payload) return {query: undefined, error: error_codes.no_payload}
        var WHERE = ''
        Object.keys(payload).forEach(key => {
            if (this._FIELDS[key]) {
                if (WHERE !== '') WHERE += ' AND '
                if (this._FIELDS[key] === 'numeric') WHERE += `"${key}" = ${payload[key]}`
