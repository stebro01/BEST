  ...
  <q-page>
    <MainSlot>
      ...
      <template v-slot:header>
        <HEADING
          :title="'Patient MultiEdit'"
          :description="'Mehrere Patienten / Eigenschaften bearbeiten'"
          :icon="'assessment'"
      ...
      <template v-slot:options_left>
        <div class="row justify-center">
          <q-btn
            ...
            no-caps
            rounded
            @click="show_dialog_select_patient = true"
            >Pat. hinzufügen</q-btn
          >
        </div>
      </template>
      <template v-slot:options_right>
        <div class="row justify-center q-gutter-md">
          <q-btn
            v-if="session_changed"
            no-caps
            rounded
            class="my-btn"
            @click="saveSession()"
            >Speichere Sitzung</q-btn
          >
          <q-btn
            v-if="session_changed"
            icon="refresh"
            ...
            @click="loadSession()"
            ><q-tooltip
              >Sitzung erneut laden / nicht gespeicherte Änderungen werden
              verworfen</q-tooltip
            ></q-btn
          >
          <q-btn
            ...
            @click="clearSession()"
            ><q-tooltip
              >Sitzung leeren -> alle lokalen Daten werden gelöscht / DB
              Einträge bleiben unverändert</q-tooltip
            ></q-btn
          >
        </div>
      </template>
      ...
      <template v-slot:main>
            ...
            <q-card class="q-pa-none">
              <q-card-actions align="center" class="q-pb-none">
                <q-btn
                  ...
                  color="dark"
                  round
                  @click="show_concept_dialog = true"
                  >add</q-btn
                >
              </q-card-actions>
              <q-card-section class="q-pb-none">
                <q-list dense>
                  <q-item
                    v-for="(item_obs, ind_obs) in OBSERVATION_TYPES"
                    :key="ind_obs + 'obstypes'"
                    clickable
                    v-ripple
                  >
                    <q-item-section side>
                      {{ item_obs.VALTYPE_CD }}</q-item-section
                    >
                    <q-item-section>
                    ...
                    <q-item-section side>
                      <q-icon
                        ...
                        name="delete"
                        @click="removeOBSERVATION_TYPE(item_obs)"
                    /></q-item-section>
                  </q-item>
                </q-list>
              </q-card-section>
            ...
            <template v-slot:body="props">
              ...
              <q-tr :props="props">
                ...
                <q-td key="AGE_IN_YEARS" :props="props">
                  ...
                  <q-popup-edit
                    v-model="props.row.AGE_IN_YEARS"
                    buttons
                    v-slot="scope"
                  >
                    <q-input
                ...
                <q-td colspan="100%">
                  <div class="absolute-top-right z-top">
                    <q-btn
                      ...
                      size="sm"
                      icon="delete"
                      ><q-tooltip>Aus der Liste entfernen</q-tooltip></q-btn
                    >
                  </div>
                  <div class="row" style="max-width: 70vw">
                    <div
                      ...
                      v-if="
                        ...
                        Array.isArray(props.row.OBSERVATIONS)
                      "
                    >
                      <!--  -->
                      <!-- CARD -->
                      <span
                        v-for="(obs, iindbs) in props.row.OBSERVATIONS"
                        :key="iindbs + 'bservations'"
                      >
                        <q-card
                          v-if="!obs._deleted"
                          class="q-pa-xs cursor-pointer"
                          @click="editPatientObservation(props.row, obs)"
                        ...
                        >
                          ...
                          <q-card-section
                            class="q-pa-none text-center text-bold"
                          >
                            {{ obs.CONCEPT_NAME_CHAR }}
                          </q-card-section>
                          <q-card-section
                            class="q-pa-none text-center bg-grey-2"
                          >
                            <span v-if="obs.VALTYPE_CD === 'N'"
                              >{{ obs.NVAL_NUM }} {{ obs.UNIT_CD }}</span
                            >
                            <span v-else-if="obs.VALTYPE_CD === 'F'">{{
                              obs.TVAL_RESOLVED
                            }}</span>
                            <span v-else-if="obs.VALTYPE_CD === 'S'">{{
                              obs.TVAL_RESOLVED
                            }}</span>
                      ...
                      <q-btn
                        icon="add"
                        @click="addPatientObservation(props.row)"
                        ><q-tooltip
                          >Eine neue Observation hinzufügen</q-tooltip
                        ></q-btn
                      >
                      <q-btn
                        @click="loadPatientObservations(props.row)"
                        icon="refresh"
                        ><q-tooltip>Neu Laden</q-tooltip></q-btn
                      >
                    </div>
                  </div>
                </q-td>
        ...
        <div class="col-12 text-center q-mt-lg" v-if="db_sync === false">
          <q-banner class="bg-red-3">DB Sync notwendig!</q-banner>
        </div>
      </template>
      ...
      <template v-slot:footer>
        <BOTTOM_BUTTONS
          :show_save="something_changed || db_sync === false"
    ...
    <!-- DIALOGS -->
    <DIALOG_SELECT_PATIENT
      v-if="show_dialog_select_patient"
      :active="show_dialog_select_patient"
      @close="show_dialog_select_patient = false"
      @save="addPatient($event)"
    />

    <CONCEPT_SELECT_NEW
      :active="show_concept_dialog"
      @close="show_concept_dialog = false"
      @save="addConcept($event)"
    />
    <CONCEPT_SELECT_NEW
      :active="show_add_observation_dialog"
      @close="show_add_observation_dialog = false"
      @save="addObservation($event)"
    />

    <EDIT_OBSERVATION_DIALOG
      v-if="show_edit_observation_dialog"
      :active="show_edit_observation_dialog"
      :item="show_edit_observation_item"
      @close="show_edit_observation_dialog = false"
      @save="saveEditedObservation($event)"
      @delete="deleteEditedObservation($event)"
    />
  </q-page>
</template>
...
export default {
  ...
  mounted() {
    ...
    this.$store.dispatch("getGender").then((res) => {
      this.options_gender = [];
      res.forEach((r) =>
        this.options_gender.push({ label: r.NAME_CHAR, value: r.CONCEPT_CD })
      );
    });
  },
  ...
  computed: {
    PATIENTS_DATA() {
      ...
      this.PATIENTS.forEach((item) => {
        if (item.OBSERVATIONS && item.OBSERVATIONS.lenght > 0)
          item.OBSERVATIONS = item.OBSERVATIONS.sort((a, b) =>
  ...
  methods: {
    ...
    addConcept(item) {
      let obj = this.OBSERVATION_TYPES.find(
        (el) => el.CONCEPT_CD === item.CONCEPT_CD
      );
    ...
    saveSession() {
      this.$store.commit("SESSION_MULTI_EDIT_SET", {
        PATIENTS: this.PATIENTS,
        OBSERVATION_TYPES: this.OBSERVATION_TYPES,
        title: "multi_edit",
        db_sync: this.something_changed === false,
      });
    ...
    removePatient(item) {
      var ind = this.PATIENTS.findIndex(
        (el) => el.PATIENT_CD === item.PATIENT_CD
      );
      if (ind > -1) {
        this.PATIENTS.splice(ind, 1);
        this.session_changed = true;
      }
    },

    removeOBSERVATION_TYPE(item) {
      var ind = this.OBSERVATION_TYPES.findIndex(
        (el) => el.CONCEPT_CD === item.CONCEPT_CD
      );
      if (ind > -1) {
        this.OBSERVATION_TYPES.splice(ind, 1);
        this.session_changed = true;
      }
    },
        ...
        for (let obs of p.OBSERVATIONS) {
          ...
          if (obs._changed === true) {
            if (!obs.OBSERVATION_ID) {
              let res_obs_add = await this.$store.dispatch("addDB", {
                query_string: obs,
              ...
              let res_obs_update = await this.$store.dispatch("updateDB", {
                query_string: { where: WHERE, set: SET },
                table: "OBSERVATION_FACT",
              });
            }
            ...
            if (obs.OBSERVATION_ID)
              await this.$store.dispatch("deleteDB", {
                query_string: { OBSERVATION_ID: obs.OBSERVATION_ID },
                table: "OBSERVATION_FACT",
              });
    ...
    async loadPatientObservations(item) {
      ...
      for (let type of this.OBSERVATION_TYPES) {
        let res = await this.$store.dispatch("searchDB", {
          query_string: {
            PATIENT_NUM: item.PATIENT_NUM,
            CONCEPT_CD: type.CONCEPT_CD,
            _view: true,
          },
          table: "OBSERVATION_FACT",
        });
      ...
      item.OBSERVATIONS = OBSERVATIONS.sort((a, b) =>
    ...
    addObservation(item) {
      ...
      if (
        this.show_add_observation_dialog_item &&
        this.show_add_observation_dialog_item.OBSERVATIONS &&
        this.show_add_observation_dialog_item.OBSERVATIONS.length > 0
      ) {
        let lastElement =
          this.show_add_observation_dialog_item.OBSERVATIONS[
            this.show_add_observation_dialog_item.OBSERVATIONS.length - 1
          ];
        if (lastElement) {
    ...
    editPatientObservation(PATIENT, OBS) {
      OBS._uid = uid(); //vergebe eine UUID um den Eintrag spaeter wieder zufinden
    ...
    saveEditedObservation(val) {
      if (!val) return;
      let obj_patient = this.PATIENTS.find(
        (el) => el.PATIENT_CD === val.PATIENT.PATIENT_CD
      );
      if (obj_patient) {
        let obj_obs = obj_patient.OBSERVATIONS.find(
          (el) => el._uid === val.OBSERVATION._uid
        );
        if (obj_obs) {
          if (val.OBSERVATION.NVAL_NUM)
            obj_obs.NVAL_NUM = val.OBSERVATION.NVAL_NUM;
          if (val.OBSERVATION.CATEGORY_CHAR)
            obj_obs.CATEGORY_CHAR = val.OBSERVATION.CATEGORY_CHAR;
          if (val.OBSERVATION.TVAL_CHAR)
            obj_obs.TVAL_CHAR = val.OBSERVATION.TVAL_CHAR;
          if (val.OBSERVATION.TVAL_RESOLVED)
            obj_obs.TVAL_RESOLVED = val.OBSERVATION.TVAL_RESOLVED;
          if (val.OBSERVATION.OBSERVATION_BLOB)
            obj_obs.OBSERVATION_BLOB = val.OBSERVATION.OBSERVATION_BLOB;
          if (val.OBSERVATION.LOCATION_CD)
            obj_obs.LOCATION_CD = val.OBSERVATION.LOCATION_CD;
          if (val.OBSERVATION.START_DATE)
            obj_obs.START_DATE = val.OBSERVATION.START_DATE;
          if (val.OBSERVATION.ENCOUNTER_NUM)
            obj_obs.ENCOUNTER_NUM = val.OBSERVATION.ENCOUNTER_NUM;
