  ...
  <q-layout view="lHh lpR fFf">
    ...
    <q-header bordered class="bg-white text-primary non-selectable">
      <q-toolbar>
        ...
        <q-btn v-if="showHomeButton" flat rounded class="absolute-top-right q-mt-sm" icon="arrow_back_ios"
          @click="this.$router.go(-1)" />
        <q-btn v-else flat rounded class="absolute-top-right q-mt-sm" icon="info"
    ...
    <!-- DRAWER -->
    <q-drawer show-if-above :mini="true" mini-to-overlay :breakpoint="500" bordered class="bg-grey-9">
      <div class="column fit q-pb-lg">
        <div class="col-10">
          <q-list padding>
            <q-item v-for="(item, ind) in essentialLinks" :key="ind + 'link'" clickable v-ripple
              @click="$router.push({ name: item.link })" :class="{ 'bg-dark': $route.path.indexOf(item.path) > -1 }">
