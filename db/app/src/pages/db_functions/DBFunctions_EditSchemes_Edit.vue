      ...
      <template v-slot:main>
        <q-card class="my-card shadow-1">
          <q-card-section>
            <div class="row q-gutter-y-sm">
              ...
              <div class="col-4 relative-position">
                <span class="absolute-center">Name🔑</span>
              </div>
              <q-input class="col-8" dense filled v-model="formData.title" />
              <!-- Beschreibung -->
              <div class="col-4 relative-position">
                <span class="absolute-center">Beschreibung</span>
              </div>
              <q-input
                class="col-8"
                dense
          ...
          <q-card-section v-if="formData.data">
            <div v-if="show_loading_spinner" class="text-center">
              <q-spinner size="md" />
            </div>
            <q-list dense bordered separator>
              <q-item
                v-for="(item, ind) in formData.data"
                :key="ind + 'item'"
                v-ripple
              >
                ...
                <q-item-section side class="cursor-pointer">
                  <div class="row q-gutter-x-sm">
                    <div>
                      <q-icon name="arrow_downward" @click="MoveUpItem(ind)" />
                      <q-icon name="arrow_upward" @click="MoveDownItem(ind)" />
                    </div>
                    <q-icon name="close" @click="removeItem(ind)" />
                  </div>
                </q-item-section>
              </q-item>
            </q-list>
...
export default {
  ...
  methods: {
    loadData() {
      ...
      this.$store
        .dispatch("searchDB", {
          query_string: { COLUMN_CD: "SCHEME_CD", CODE_CD: id },
          table: "CODE_LOOKUP",
        })
        .then((res_scheme) => {
    ...
    async resolveConceptData(data) {
      ...
      for (let item of data) {
        if (item.CONCEPT_CD) {
          let concept = await this.$store.dispatch("searchDB", {
            query_string: item,
            table: "CONCEPT_DIMENSION",
          });
          if (concept.length > 0) {
            this.formData.data.push({
              CONCEPT_CD: concept[0].CONCEPT_CD,
              NAME_CHAR: concept[0].NAME_CHAR,
              VALTYPE_CD: concept[0].VALTYPE_CD,
            });
          }
        }
      }
    ...
    async removeItem(ind) {
      if (!(await my_confirm(`Eintrag wirklich löschen?`))) return;
      this.formData.data.splice(ind, 1);
    },

    MoveDownItem(ind) {
      this._swap(this.formData.data, ind-1, ind);
    },

    MoveUpItem(ind) {
      this._swap(this.formData.data, ind, ind+1);
    },

    _swap(arr, index, index2) {
    ...
    saveScheme(val) {
      ...
      if (!valid)
        return this.$q.notify(this.$store.getters.TEXT.alerts.form_not_valid);
      //else
      const data = [];
      ...
      this.$store
        .dispatch("searchDB", {
          query_string: { COLUMN_CD: "SCHEME_CD", CODE_CD: SCHEME.CODE_CD },
          table: "CODE_LOOKUP",
        })
        .then((res_scheme) => {
          if (res_scheme.length > 0) {
            var WHERE = {
              CODE_CD: SCHEME.CODE_CD,
              COLUMN_CD: SCHEME.COLUMN_CD,
            };
            var SET = SCHEME;
            ...
            this.$store
              .dispatch("addDB", { query_string: SCHEME, table: "CODE_LOOKUP" })
              .then((res) => {
                this.$q.notify(this.$store.getters.TEXT.msg.action_successful);
                this.$router.push({ name: "DBFunctions_EditSchemes" });
              })
    ...
    conceptSelected(val) {
      let obj = this.formData.data.find(
        (el) => el.CONCEPT_CD === val.CONCEPT_CD
      );
      if (!obj)
        this.formData.data.push({
          CONCEPT_CD: val.CONCEPT_CD,
          NAME_CHAR: val.NAME_CHAR,
          VALTYPE_CD: val.VALTYPE_CD,
        });
