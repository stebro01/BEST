  ...
  <q-page>
    <MainSlot :no_footer="true">
      ...
      <template v-slot:main>
        <div class="row justify-center q-gutter-xl">
          <!-- UNTERSUCHER -->
          <q-card
            v-if="$store.getters.PROVIDER_PINNED === undefined"
            class="my-card"
          >
            <q-item>
              <q-item-section avatar>
                <img
                  src="~assets/provider-color-logo.png"
                  class="my-icon-size"
                />
              </q-item-section>
              <q-item-section>
          ...
          <!-- SELECT PATIENT -->
          <q-card
            v-if="$store.getters.PATIENT_PINNED === undefined"
            class="my-card"
          >
            <q-item>
              <q-item-section avatar>
                <img
                  src="~assets/patient-color-logo.png"
                  class="my-icon-size"
                />
              </q-item-section>
              <q-item-section>
    ...
    <q-dialog v-model="select_patient_dialog">
      <q-card class="my-card">
        ...
        <SELECT_LIST_CARD_SECTION
          :list="select_patient_list"
          @save="patientSelectedFromDialog($event)"
        />
        ...
        <q-card-actions align="right">
          <q-btn
            ...
            rounded
            flat
            :label="$store.getters.TEXT.btn.close"
            color="primary"
            v-close-popup
          />
        </q-card-actions>
...
export default {
  ...
  methods: {
      ...
      this.$store
        ...
        .then((res) => {
          if (res.length === 1) {
            ...
            this.$q.notify(
              `Patient mit << ${JSON.stringify(PATIENT)} >> nicht gefunden`
            );
        })
    ...
    check_status() {
      if (
        this.$store.getters.PROVIDER_PINNED &&
