...
export class View_X {
  ...
  _SCHEME = undefined;
  _RESOLVE_CONCEPT_CD = [
    { FIELD: "SEX_CD", CONCEPT_CD: "SCTID: 263495000" },
    { FIELD: "RACE_CD", CONCEPT_CD: "LID: 46463-6" },
    { FIELD: "MARITAL_STATUS_CD", CONCEPT_CD: "LID: 45404-1" },
    { FIELD: "LANGUAGE_CD", data: ["german", "english", "other"] },
  ];
  ...
  constructor(db_man, db_fn, upload_id) {
    if (db_man === undefined || !db_fn) {
      log({
        method: `ERROR: ${this._CLASS_NAME} -> constructor`,
        message:
          "payload is empty, no refrence to db_man.js or the db_filename",
      });
      return;
    }
  ...
  async create(payload) {
    ...
    if (primary_key_is_to_be_set) {
      const status_pk = await this._check_primary_key(
        payload[this._SCHEME._PRIMARY_KEY]
      );
    ...
    if (this._SCHEME._NOT_NULL) {
      ...
      if (!check_not_null) {
        log({
          method: `${this._CLASS_NAME} -> create`,
          message: `must not be undefined: ${JSON.stringify(
            this._SCHEME._NOT_NULL
          )}`,
          data: JSON.stringify(payload),
        });
        return {
          error: `${error_codes.invalid_payload}:: ${JSON.stringify(
            this._SCHEME._NOT_NULL
          )} must be defined`,
        };
      }
  ...
  async update(payload) {
    ...
    if (!payload) return { error: error_codes.no_payload };
    if (!payload.where || !payload.set)
      return { error: error_codes.invalid_payload };

    //payload.set should be the primaryKey
    if (!payload.where[this._SCHEME._PRIMARY_KEY])
      return { error: error_codes.invalid_payload };
  ...
  async delete(payload) {
    ...
    if (!payload) return { error: error_codes.no_payload };
    //payload.set should be the primaryKey
    if (!payload[this._SCHEME._PRIMARY_KEY] && !payload._force)
      return { error: error_codes.invalid_payload };
  ...
  async resolve_cd() {
    ...
    for (let i = 0; i < this._RESOLVE_CONCEPT_CD.length; i++) {
      ...
      if (el.data) result[el.FIELD] = el.data;
        ...
        let sql_query = SCHEME_CONCEPT_DIMENSION.read({
          CONCEPT_CD: el.CONCEPT_CD,
        });
        if (!sql_query.error) {
          ...
          if (!res.error && res.data.length > 0) {
            ...
            if (CONCEPT_PATH) {
              let path_query = SCHEME_CONCEPT_DIMENSION.read({
