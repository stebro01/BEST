<template>
  <div class="row justify-center q-mt-xl" >
    <div class="col-12 text-center ">DRUCK VORSCHAU
      <q-btn round flat size="lg" @click="printDiv('myPrintDiv')" icon="picture_as_pdf"><q-tooltip>{{ $store.getters.TEXT.btn.tooltip.pdf }}</q-tooltip></q-btn>
            ...
            <div class="row q-gutter-xs">
              ...
              >
                ...
                <q-card-section  class="q-pa-none">
                  <span v-if="obs.NVAL_NUM">{{obs.NVAL_NUM}} <span v-if="obs.UNIT_CD">{{obs.UNIT_CD}}</span></span>
                  <span v-else-if="obs.TVAL_RESOLVED">{{obs.TVAL_RESOLVED}}</span>
...
export default {
  ...
  methods: {
    async prepareData(data) {
      ...
      const VISITS = []
      const ENCOUNTERS = Array.from(new Set(data.map(item => item.ENCOUNTER_NUM)))
      for (let ENCOUNTER_NUM of ENCOUNTERS) {
        let OBJ = data.filter(el => el.ENCOUNTER_NUM === ENCOUNTER_NUM)
        if (OBJ) {
          let V = {observation: [], surveybest: [], info: {ENCOUNTER_NUM: ENCOUNTER_NUM}}
          for (let obj of OBJ) {
