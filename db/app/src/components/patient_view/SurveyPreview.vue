<template>
  ...
  export default {
    ...
    mounted() {
        ...
        this.$store.dispatch('searchDB', {table: 'OBSERVATION_FACT', query_string: {OBSERVATION_ID: this.input_data.OBSERVATION_ID, _columns: ['OBSERVATION_BLOB']}})
        .then(res => {
            
            if (res.length > 0) this.previewSurveyBest(res[0].OBSERVATION_BLOB)
        })
    },
