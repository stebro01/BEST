<template>
    ...
    <div  class="row justify-center bg-white" style="max-width: 90%" id="mySurveyDIV">
      <q-card  class="q-ma-md no-shadow" v-if="preview_raw_item">
        <q-icon class="float-right z-top cursor-pointer q-ml-md" @click="$emit('close')" name="close" size="md"><q-tooltip>{{ $store.getters.TEXT.btn.tooltip.close }}</q-tooltip></q-icon>
          <q-card-section>
        ...
        <q-card class="q-ma-md no-shadow" v-if="preview_survey_best_item">
          ...
          <q-card-section v-if="preview_survey_best_item.SUMMARY">
            <q-item-label v-for="(item, ind) in Object.keys(preview_survey_best_item.SUMMARY)" :key="ind + 'summary'"
              ...
              <span v-if="ind > 0">{{ preview_survey_best_item.SUMMARY[item].label }}</span>
          ...
          <div class="row">
            <div class="col-6">
              <q-card-section v-if="preview_survey_best_item.DESCRIPTION">
                <q-item-label v-for="(item, ind) in Object.keys(preview_survey_best_item.DESCRIPTION)"
                  :key="ind + 'DESCRIPTION'">
          ...
          <q-card-section v-if="preview_survey_best_item.RESULTS">
            <q-table hide-header hide-bottom grid dense
              :rows="preview_survey_best_item.RESULTS.filter(el => el.label !== 'PID' && el.label !== 'quest' && el.label !== 'date')"
  ...
  export default {
    ...
    mounted() {
        ...
        .then(res => {
            if (res.length > 0) {
              if (res[0].CATEGORY_CHAR === 'surveyBEST') this.previewSurveyBest(res[0].OBSERVATION_BLOB)
