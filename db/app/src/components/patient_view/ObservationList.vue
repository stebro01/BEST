            ...
            <q-markup-table dense separator="cell" style="max-width: 100% !important">
                ...
                <thead>
                    <tr>
                        <!-- EDIT_MODE -->
                        <th v-show="EDIT_MODE">
                            <q-btn flat size="sm" icon="drag_indicator"
                                @click="show_drop_list = true"><q-tooltip>Reihenfolge ändern</q-tooltip></q-btn>
                        </th>
                        <!-- VALUES -->
                    ...
                    <tr v-for="(item, index) in OBSERVATIONS" :key="index + 'obs'" :id="`box_${index}`">
                        ...
                        <td class="col_DATA" :class="{
                            ...
                            <POPUP_DATA v-if="EDIT_MODE &&
                                    ...
                                    item._edit_val
                                    " :item="item" @update="updateValue($event, item)" @close="item._edit_val = false" />
                        </td>
                        <!-- UNIT -->
                        <td class="col_UNIT_CD" :class="{
                            ...
                            }}<q-tooltip>{{ item.UNIT_RESOLVED || item.UNIT_CD }}</q-tooltip>
                        ...
                        <td class="col_CATEGORY_CHAR" :class="{
                            ...
                            <span v-if="item.CATEGORY_CHAR === 'RAW' ||
                                ...
                                " class="cursor-pointer" @click="downloadRAW(item)"><q-icon
                                    name="download"><q-tooltip>Download</q-tooltip></q-icon></span>
                            <span v-if="item.CATEGORY_CHAR === 'surveyBEST'" class="cursor-pointer"
                                @click="previewSURVEY(item)"><q-icon
                                    name="preview"><q-tooltip>Preview</q-tooltip></q-icon></span>
                            {{ item.CATEGORY_CHAR }}
                            <POPUP_CATEGORY v-if="EDIT_MODE &&
                                item._edit_cat &&
                                !FORBIDDEN_CATEGORY.includes(item.CATEGORY_CHAR)
                                " :item="item.CATEGORY_CHAR" @update="updateCategory($event, item)" @close="item._edit_cat = false" />
                        </td>
