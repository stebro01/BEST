        ...
        >
          ...
          >
            <div class="q-px-md text-center">
              ...
              <div
                v-if="item.code === 'patient'"
                class="cursor-pointer"
                @click="gotoPatient(item.data)"
              >
                {{ item.data[item.field] }}
              </div>
              <div
                v-else-if="item.code === 'visit'"
                class="cursor-pointer"
                @click="$router.push({ name: 'VisitsView' })"
              >
                {{ item.data[item.field] }}
              </div>
...
export default {
      ...
      const data = [
        {
          code: "provider",
          label: "Provider",
          class: "my-provider-color",
          icon: "face",
          data: this.$store.getters.PROVIDER_PINNED,
          field: "NAME_CHAR",
        },
        {
          code: "patient",
          label: "Aktiver Patient",
          class: "my-patient-color",
          icon: "person",
          data: this.$store.getters.PATIENT_PINNED,
          field: "PATIENT_CD",
        },
        {
          code: "visit",
          label: "Visite",
          class: "my-visit-color",
          icon: "event",
          data: this.$store.getters.VISIT_PINNED,
          field: "START_DATE",
        },
        {
          code: "observation",
          label: "Observation",
          class: "my-observation-color",
          icon: "fact_check",
          data: this.$store.getters.OBSERVATION_PINNED,
          field: undefined,
        },
      ];
  ...
  methods: {
    ...
    chipRemove(val) {
      switch (val) {
        case "provider":
          if (this.$route.fullPath.indexOf("/visits") >= 0)
    ...
    gotoPatient(PATIENT) {
      this.$router.push({
        name: "Patients_Edit",
        params: { PATIENT_NUM: PATIENT.PATIENT_NUM },
      });
    },
