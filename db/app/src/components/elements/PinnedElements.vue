<template>
  <div class="z-top">
    <q-fab v-if="$store.getters.PROVIDER_PINNED || $store.getters.PATIENT_PINNED" v-model="DRAWER" label="Actions"
      external-label flat vertical-actions-align="left" class="text-black my-visit-color" icon="more_vert"
      active-icon="more_horiz" direction="down" :hide-label="hideLabels" square>
      <span v-for="(item, ind) in BUTTON_ROW" :key="ind + 'btn'">
        <q-fab v-if="item.data" :class="item.class" class="q-ml-xs q-mb-sm" round flat padding="xs"
          @click="onClick(item.code)" :icon="item.icon">
          <q-chip :class="item.class" class="q-px-lg" removable @remove="chipRemove(item.code)" text-color="white"
            :icon="item.icon">
