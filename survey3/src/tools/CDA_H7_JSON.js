...
function prepare_evaluation(cda, data) {
  ...
  data.quest.results.forEach(item => {
    if (item.evaluation !== undefined) evaluation.text.div += `<div id="section_evaluation">${item.evaluation}</div>`
  })
...
function prepare_row(data) {
  ...
  header.push('PID'); row.push(data.PID)// PID
  header.push('quest'); row.push(data.quest.label)// QUEST
